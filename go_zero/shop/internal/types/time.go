...
func (t *JSONTime) Scan(v interface{}) error {
	...
	if value.Location() == nil || value.Location().String() == "" {
		...
		value = value.In(loc)
		value = value.Add(-time.Hour * 8) //postgresql无时区时间 有差别,待验证
	}
	if ok {
...
func (t *JSONTime) GetParkingLengthMinute(left *JSONTime) int {
	spanSeconds := int64(left.Sub(t.Time).Minutes())
	return int(spanSeconds)
}
